...
if [[ "$count" != "1" || "${executed[0]}" != "$target_output" ]]; then
  echo "Expected only $target_output to rebuild after nested util change." >&2
  echo "Executed targets:" >&2
  printf '  %s\n' "${executed[@]}" >&2
  exit 1
fi
...
cat > out/dep-04-build-discovery.json <<EOF
{
  ...
  "nested_change_rebuilt_target": true,
